...
## Автономный режим (сервер на месяц)

Скрипт рассчитан на `nohup` / systemd / screen: сам переживает 429, таймауты и падение воркера сети.

```bash
nohup python scan_defi.py --chains ethereum,bsc,polygon,zk,zksync,robinhood,hyperliquid > /dev/null 2>&1 &
```

При старте в консоль печатается таблица: `start_block`, `last_indexed`, текущий `head`, сколько блоков осталось, сколько контрактов уже в БД. Дальше консоль почти молчит.

| файл | зачем |
|------|--------|
| `logs/scanner.log` | основной лог, ротация 20 МБ × 14 |
| `logs/errors.log` | warning+ |
| `logs/status.txt` | живая таблица прогресса, перезаписывается каждые 10 мин |
| `reports/qualifying.xlsx` | перезаписывается каждые 5 мин |
| `reports/below_threshold.xlsx` | то же |
| `reports/archive/qualifying_YYYYMMDD.xlsx` | суточный снимок |

RPC: при 429 / CU limit / 502 / timeout нода уходит в экспоненциальный кулдаун (5с → максимум 15 мин), запросы идут на следующий URL. Если все ноды сети лежат — поток этой сети спит и потом продолжает с того же блока. Размер батча сам сжимается и отрастает.

Упавший воркер сети поднимается через `task_restart_sec` (20с). SQLite резюмируется с `last_indexed`.

## Запуск

Обе очереди сразу (индексатор + чекер балансов + Excel):
