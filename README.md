...
- wrong chain отключает endpoint; HTTP 401/403 на отдельном методе отключает этот метод, а лимит квоты включает cooldown;
...
- если отсутствует блок или обязательный receipt, discovery cursor не двигается;
- `Transfer`-логи обрабатывает отдельная сохраняемая очередь: её ошибка не блокирует discovery cursor;
...
Первый скан каждого адреса проверяет все поддерживаемые сети. Далее подтверждённые
`qualifying`-контракты проверяются через 6 часов, балансы от 10% порога — через
24 часа, меньшие положительные — через 72 часа, нулевые — через 7 дней, а сети
без bytecode — через 30 дней. RPC-ошибки и частичные сканы повторяются с
задержкой 10 минут → 30 минут → 2 часа → 6 часов → 24 часа; отсутствие цены —
через 6 часов, карантинные оценки — через 24 часа. Новая связь контракта с
токеном ускоряет проверку только этой сети. Ни один адрес не исключается навсегда.
`recheck_interval_sec` оставлен в YAML лишь для совместимости со старой
конфигурацией и не управляет этим расписанием. При большой очереди фактический
повтор может начаться позже. Лог `[balances]`
...
| `token_log_tasks` | отдельная позиция и состояние поиска `Transfer` по каждому приоритетному контракту |
...
- Очередь токеновых логов ограничена 30 RPC-попытками в минуту суммарно и четырьмя на сеть. Новый контракт получает задачу при коммите discovery; существующие контракты добавляются при миграции только с подтверждённым балансом от 10% порога.
- Для `active_call` история до первого наблюдаемого вызова неизвестна. При частичном токеновом покрытии подтверждённая сумма остаётся нижней оценкой: ниже порога статус `incomplete`, выше порога — `qualifying` с пометкой.
